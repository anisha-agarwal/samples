...
void handleConnection(Socket* s, Socket r1)
{
  ...
  while(1)
	...
	break;
      ...
      Packet* ack = new Packet("Router1",1,1,1,"ack");
      char ack_str[512];
      ...
      p->SetIdString("Router1");       // Changing the string identifier to Router1
  ...
  printf("Completed Phase 2 for Router1\n");
}
...
int main()
{
  ...
  i = r1.Sock_Name();
  printf("Router1 has TCP port 21219 and UDP port %d\n",i);
  char* IP_Add;
  p.Bind(21219,"nunki.usc.edu");                 // Binding the TCP socket to the static port 21219
  IP_Add = p.ip();
  printf("Router1 has IP Address %s\n",IP_Add);
  p.Listen();                                    // Listening for incoming requests
  s = p.Accept("nunki.usc.edu");                 // Accepting the incoming connection 
  printf("Router1 accepted connection request from Messenger\n");
