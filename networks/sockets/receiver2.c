...
int main()
{
  ...
  s.Bind(3319,"nunki.usc.edu");                        //Binding the static UDP socket
  r1.Bind(0,"nunki.usc.edu");
  int sock_no;
  sock_no = r1.Sock_Name();
  printf("Receiver2 has UDP ports - static 3319 and dynamic %d \n",sock_no);
  char* IP_Add;
  IP_Add = s.ip();
  printf("Receiver2 has IP Address %s \n",IP_Add);
  int i,n,m;
  vector<char*> words;
  char* word;
  string sent;
  ...
  printf("End of Phase 2 for Receiver2 \n");
  ...
  Packet P_S("Receiver2",1,1,1,sent.c_str());
  //P_S.print_packet();
  P_S.to_string(data,1024);
  r1.Sendto(data,strlen(data),4219,"nunki.usc.edu");
  printf("Forwarded Sentence2 to messenger over UDP\n");
  printf("End of phase 3 for Receiver2 \n");
  r1.Close();
  s.Close();
}
 over UDP\n",n+1,word,id_string);
